...
def get_user_location():
    """Fetches accurate user location using Google Geolocation API with error handling"""
    try:
        gmaps = googlemaps.Client(key=GOOGLE_MAPS_API_KEY)
        location = gmaps.geolocate()
        
        if 'location' not in location:
            st.error("Could not determine location. Using default coordinates.")
            return 28.7041, 77.1025  # Default to New Delhi coordinates
        
        return location['location']['lat'], location['location']['lng']
    
    except Exception as e:
        st.error(f"Location service error: {str(e)}. Using default coordinates.")
        return 28.7041, 77.1025  # Default to New Delhi coordinates
...
def get_nearby_hospitals(lat, lon):
    ...
    places_result = gmaps.places_nearby(
        location=(lat, lon), radius=5000, type='hospital'
    )
    return places_result.get("results", [])       

def get_nearby_counselors(lat, lon):
    """Fetch nearby mental health services and counselors using Google Places API"""
    google_api_key = "YOUR_GOOGLE_API_KEY"
    search_query = "mental health counselor"
...
#Chatbot Section
    ...
    with tab2:
        ...
        if counselors:
            ...
            st.warning("No mental health services found nearby.")
